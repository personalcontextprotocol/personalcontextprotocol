# Changelog

All notable changes to PCP will be documented in this file.

The format is based on Keep a Changelog, and this project uses semantic version
numbers for release tags where practical.

## [Unreleased]

### Changed

- Consolidated the TypeScript and Rust SDK sources under `sdk/typescript` and
  `sdk/rust` while preserving the existing package and crate names.

## [0.1.0] - 2026-06-26

### Added

- Initial PCP v0.1 alpha protocol proposal.
- TypeScript/Zod protocol schemas and generated JSON Schema.
- Fastify reference server configured for local development with SQLite persistence.
- TypeScript client and runnable demo client.
- Consent grants, scoped context requests, ContextPacks, memory proposals, direct memory writes/deletes, scoped audit listing, exports, and audit logs.
- Public alpha governance files, GitHub templates, and CI workflow.
- Whitepaper draft for v0.1 review.

### Notes

- v0.1 is an alpha proposal and reference implementation, not a finished standard.
- The reference server is intended for local development and protocol validation, not production authentication.
